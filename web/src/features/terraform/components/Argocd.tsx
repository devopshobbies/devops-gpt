...
const Argocd = () => {
  ...
  const { data, isPending, isError, isSuccess, mutate } = useMutation<
    ...
    Error,
    {
      body: {
        argocd_application: {
          sync_policy: { auto_prune: boolean; self_heal: boolean };
        } | null;
  ...
  const handleSubmit = async () => {
    const body = {
      argocd_application: menus.argocd_application
        ? {
            sync_policy: {
  ...
  return (
    <div className="flex flex-col items-center justify-center">
        ...
        <div className="divide-y-2 divide-gray-300">
          <div className="flex items-center justify-between w-full px-4 py-2">
            ...
            <input
              type="checkbox"
              className={cn(
                "border-orange-[#2e323a] toggle [--tglbg:#2e323a] bg-orange-300 hover:bg-orange-400",
                {
                  "bg-[#5b6372] hover:bg-[#5b6372]": !menus.argocd_application,
                }
              )}
              checked={menus.argocd_application}
              onChange={() => {
          ...
          <div
            className={cn(
              "divide-y-2 divide-gray-300 max-h-0 overflow-hidden transition-all ease-linear duration-300",
              { "max-h-96": menus.argocd_application }
            )}
          >
            <button
              className="flex items-center justify-between w-full py-2 pl-10 pr-4"
              onClick={() => handleMenus("sync_policy")}
            >
              <p>Sync Policy</p>
              <FaChevronDown
                className={cn("transition-all", {
                  "rotate-180": menus.sync_policy,
                })}
              />
            </button>
            <div
              ...
              <div className="py-2 pl-16 pr-4">
                <div className="flex items-center justify-between">
                  ...
                  <input
                    ...
                    checked={buttons.auto_prune}
                    onChange={() => handleButtons("auto_prune")}
                  />
                </div>
              </div>
              <div className="py-2 pl-16 pr-4">
                <div className="flex items-center justify-between">
                  ...
                  <input
                    ...
                    checked={buttons.self_heal}
                    onChange={() => handleButtons("self_heal")}
                  />
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between">
              ...
              <input
                ...
                className={cn(
                  "border-orange-[#2e323a] toggle [--tglbg:#2e323a] bg-orange-300 hover:bg-orange-400",
                  {
                    "bg-[#5b6372] hover:bg-[#5b6372]":
                      !buttons.argocd_repository,
                  }
                )}
                checked={buttons.argocd_repository}
                onChange={() => handleButtons("argocd_repository")}
              />
            </div>
          </div>
          <div className="px-4 py-2">
            <div className="flex items-center justify-between">
              ...
              <input
                ...
                className={cn(
                  ...
                  {
                    "bg-[#5b6372] hover:bg-[#5b6372]":
                      !buttons.application_depends_repository,
                  }
                )}
                checked={buttons.application_depends_repository}
                onChange={() => handleButtons("application_depends_repository")}
              />
            </div>
          </div>
        </div>
      ...
      <button
        ...
        className="w-full py-2 mt-4 text-center transition-all bg-orange-800 rounded-md btn max-w-96 hover:bg-orange-900"
      >
        {isPending ? (
          <div className="flex items-center justify-center gap-4 disabled:opacity-80">
